...
var boolAnalyse = false;

var noteTotal = 0;
var nbJoueeJuste = 0;
var frequenceAttendue = new Array(); // A envoyer en parametre à l'appel de la fonction

function evaluerFrequenceDuree(pPrequence)
{
	frequenceAttendue = pFrequence;
	analyse();
}
...
function analyse()
{
	...
	if(!boolAnalyse && noteTotal != 0)
	{
		nbJuste=0;
 		duree=0;
 		alert('déduction note ' + nbJoueeJuste*100/noteTotal);
	}
	else if(boolAnalyse)
	{
		console.log('analyse en cour');
	    updatePitch();			
	}
	//peut-être récusivité à ajouter
}

function updatePitch( time ) {
	//analyse d'une note supplémentaire
	noteTotal++;

	//récupération des notes jouées dans un buffer
 	...
 	*/
 	if (ac == -1 || ac >= 1500) {
		...
		noteElem.innerText = "--";
	 	...
	 	frequenceAttendue.forEach(function(value)
	 	{
	 		var borneFreqMin = frequenceAttendue * 0.9;
			var borneFreqMax = frequenceAttendue * 1.1;
			if(value > borneFreqMin && value < borneFreqMax)
			{
				nbJoueeJuste++;
			}
	 	});
...
* la note attendue.
*/
function correspondanceNote(frequenceJouee)
{
	//borne d'erreur de 20%
	var borneFreqMin = frequenceAttendue * 0.9;
	var borneFreqMax = frequenceAttendue * 1.1;
	if(frequenceJouee > borneFreqMin && frequenceJouee < borneFreqMax)
	{
		nbJoueeJuste++;
	}
	//objetnote ?
...
function correspondanceAccord(frequenceJouee)
{
